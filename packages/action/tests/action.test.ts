...
describe('action registry integrity', () => {
  it('contains all expected actions', () => {
    ...
    expect(ids).toEqual(
      ['WAKE_UP', 'GO_TO_SCHOOL', 'STUDY_AT_SCHOOL', 'GO_HOME', 'IDLE_AT_HOME', 'SLEEP'].sort()
    );
  });
});
...
describe('GO_TO_SCHOOL', () => {
  ...
  it('executor moves to SCHEOOL and sets activity STUDY_AT_SCHOOL', () => {
    const meta = getAction('GO_TO_SCHOOL')!;
    meta.executor();
...
describe('STUDY_AT_SCHOOL', () => {
  it('precondition passes at SCHEOOL during school hours and not already studying', () => {
    const meta = getAction('STUDY_AT_SCHOOL')!;
    charactorState.setLocation(WorldLocation.SCHEOOL);
  ...
  it('precondition fails when not at SCHEOOL', () => {
    const meta = getAction('STUDY_AT_SCHOOL')!;
    charactorState.setLocation(WorldLocation.HOME);
    worldState.updateTime(dayjs().hour(timeConfig.scenes.schoolWindow.startHour).minute(0));
    expect(meta.precondition()).toBe(false);
...
describe('GO_HOME', () => {
  it('precondition passes at SCHEOOL when hour >= goHomeAfterHour', () => {
...
describe('IDLE_AT_HOME', () => {
  it('precondition passes when at HOME and not already idle', () => {
    const meta = getAction('IDLE_AT_HOME')!;
    charactorState.setLocation(WorldLocation.HOME);
    charactorState.setActivity(Activity.WAKE_UP);
    expect(meta.precondition()).toBe(true);
  });
